...
pub fn init_scheme() -> Result<()> {
    ...
    let (clash, _) = hkcu.create_subkey("Software\\Classes\\clash-smart")?;
    clash.set_value("", &"Clash Verge Smart")?;
    clash.set_value("URL Protocol", &"Clash Verge Smart URL Scheme Protocol")?;
    let (default_icon, _) = hkcu.create_subkey("Software\\Classes\\clash-smart\\DefaultIcon")?;
    default_icon.set_value("", &app_exe)?;
    let (command, _) = hkcu.create_subkey("Software\\Classes\\clash-smart\\Shell\\Open\\Command")?;
...
#[cfg(target_os = "linux")]
const DEEP_LINK_SCHEMES: &[&str] = &["clash-smart", "clash-verge-smart"];
