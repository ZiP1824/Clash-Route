...
- 新增智能服务分流，内置 ChatGPT、Claude、Gemini、GitHub、YouTube、Netflix、Steam 等 12 项服务模板
- 支持为不同服务绑定独立策略或节点，并按服务、用户规则、分类规则、订阅规则的顺序生成配置
- 新增智能分流连接监控，可查看服务匹配、实际代理链与预期策略
- 新增自动纠错原型，在高可信域名或进程匹配出现偏差时重新应用配置并重建相关连接
- 策略选择器新增来源筛选、名称排序、延迟排序和批量延迟测试

### 🚀 优化改进

- 自定义域名规则支持填写多个值
- 完善跨订阅节点引用和链式代理的 `dialer-proxy` 生成
- 增加智能分流配置的导入、导出和服务级保存入口
- 首页增加智能分流状态卡片，并完善旧配置兼容与类型定义
